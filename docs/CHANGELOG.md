...
Two big arrivals on the user-visible side: **local voice input** (push-to-talk via on-device whisper.cpp, macOS first) and the **collection instant-present** flow that pops the collection's canvas card the moment a slash-command chat references it — no waiting for the agent to finish. Under the hood this release is dominated by the **plugin-extraction sweep** that started with chart / form and now reaches `presentCollection` and `presentHtml`: both surfaces' server core, Vue View / Preview, and 8-locale i18n all move into standalone npm packages (`@mulmoclaude/collection-plugin` 0.3 → 0.5.2, `@mulmoclaude/html-plugin` 0.1 → 0.2.2) consumable by MulmoTerminal with no MulmoClaude code changes. Whisper input also lands as a shared `@mulmoclaude/whisper` package. Side dishes: a new `mc-zenn` preset skill, several path-traversal hardening fixes, Windows `claude.exe` spawn fix, a Docker MCP broker path fix, and CI dev-server pre-warming to de-flake e2e `page.goto`.

### Added
- **Local voice input — push-to-talk via on-device whisper.cpp** (#1773 + #1775) — toggle the mic icon in chat input, hold to talk, release to send. Audio is streamed to a sidecar `whisper.cpp` process bundled with the launcher, so transcription stays on-device (no cloud TTS / STT). macOS is the first-class target. Auto-resume per turn on a sticky session, pause-based segmentation, single-flight guard on mic start, in-memory armed-mic reset on session change. Extracted as the shared `@mulmoclaude/whisper` package so MulmoTerminal can reuse the same core.
- **Collection instant-present** (#1785) — when a slash-command chat references a collection, the collection's canvas card now appears **instantly** on the chat draft path (before the agent finishes its first round), not after. Also covered: synthetic-seed guard against a fast-agent race when the synth-seed arrives while the agent is still booting (`fix(collections): guard synthetic seed against fast-agent race`).
- **`mc-zenn` preset skill** (#1786) — one of the bundled `mc-*` presets to publish work as a Zenn article (the Japanese dev-blog platform). Bundled in `@mulmoclaude/workspace-setup@0.1.8`. Discover via the skills launcher.
- **`@mulmoclaude/whisper` package** (#1775) — new shared npm package extracting the whisper.cpp sidecar core. The MulmoClaude launcher declares it as a dependency (`@mulmoclaude/whisper`) instead of carrying the sidecar code inline; MulmoTerminal can import the same package directly. Model validation, graceful shutdown, stale-error handling all live in the package.
- **`@mulmoclaude/html-plugin@0.1.0 → 0.2.2`** — NEW shared package for the `presentHtml` tool. **0.1.0**: server-core extraction (schema + save / validate logic) against the generic gui-chat-protocol `files.artifacts` capability, the MulmoClaude host shrinks to thin adapters. **0.2.0**: Vue View / Preview + `./style.css` move into the package's `./vue` entry, MulmoClaude's `src/plugins/presentHtml/` loses 449 lines and its i18n keys move into the package's own 8 locales. **0.2.1 / 0.2.2**: review hardening — the "PDF" print button uses the same `previewUrl ?? htmlArtifactPreviewUrl(filePath)` fallback as the iframe; `executeHtmlDispatch` validates `path` / `html` are strings before touching FileOps; `htmlArtifactPreviewUrl` rejects `..` / empty path segments. No user-facing change in MulmoClaude, but MulmoTerminal can now run presentHtml end-to-end.
- **`@mulmoclaude/collection-plugin@0.3.0 → 0.5.2`** — sweep of extraction phases. **0.3.0**: six View components (`CollectionRecordModal`, `CollectionEmbedView`, `CollectionCalendarView`, `CollectionDayView`, `CollectionKanbanView`, `CollectionRecordPanel`) move into the package's `./vue` entry behind a `configureCollectionUi()` host binding. **0.4.0**: extraction COMPLETE — every View component (incl. `CollectionView`, the record / config / custom-view modals, and the browsable `CollectionsIndexView` / `FeedsView` index pages) plus the plugin's OWN self-contained vue-i18n instance (all 8 locales) live in the package. **0.4.1**: missing `common.close` i18n key + locale-sync hardening. **0.5.0**: consumable by a router-less host — refs / file cell links + record modal teleport go through host bindings instead of `<router-link>` / `body` teleport. **0.5.1**: keyboard access + `navigate`-absent guard on those host-bound links (in `src/vue/refLink.ts`). **0.5.2**: record ids accept interior dots (new `safeRecordId`), so natural keys like a Slack ts (`1718900000.123456`) or a SemVer (`1.2.3`) are addressable via `manageCollection` (#1735); `..`, path separators, and leading / trailing dots stay rejected.
- **`@mulmoclaude/collection-watchers@0.1.1`** — `trigger date unparseable` now warns only for a present-but-malformed value; an absent / empty optional trigger date is suppressed silently instead of logging a WARN every reconcile tick.
...
- **e2e dev-server pre-warming** (`test(e2e): pre-warm Vite dev server in globalSetup to de-flake page.goto`) — Playwright's `globalSetup` now warms the Vite dev server before any test starts, so the first `page.goto` doesn't pay the on-demand compile penalty and the e2e `(1)` / `(2)` shards stop occasionally timing out on the first navigation.
- **`tsconfig.json` `types: ["vite/client", "node"]`** — adds `"node"` so `vue-tsc` resolves the `node:*` imports in `src/lib/wiki-page/*` (which is consumed only by server-side code but lives under `src/`). Unblocks the lint_test typecheck step on the lockfile-only PRs.
...
- **MCP broker path in Docker** (#1771 / #1770) — the broker source path resolves relative to `config.ts` and the same fix applies under Docker bind mounts too.
- **Attachment companion-file traversal** (#1756 + #1760 + #1762 + #1765) — path validator factory + `..` strictness restoration so attachment companion files can't escape their parent dir, with a consolidated traversal-segment check shared across the host. Closes a class of issues opened during the chat-input multi-attach work in 0.8.0.

### Docs
- **`collections-vibe-crafting-help.md`** (#1758) — new help doc on the iterative "vibe-craft a collection from a sample" workflow.

### Refactor
- **frontend `toError` helper** (#1766) — single helper for `unknown → Error` narrowing on the frontend; an `errorMessage` codemod sweep (#1767) standardises the existing patterns over the codebase. No behaviour change. `CollectionView` + its two config/custom-view modals follow in a later pass.
