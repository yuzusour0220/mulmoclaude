...
| `packages/core/src/collection/core/dynamicIcon.ts` (`@mulmoclaude/core/collection`) | `selectDynamicRecord(records, source, orderBy)` / `resolveIcon(record, spec, staticIcon)` / `firstDateField(schema)` | Pure resolver for `CollectionSchema.dynamicIcon` (data-driven launcher icon): pick the deciding source record, then map it through the first-match-wins `rules`. Server wraps it in `packages/core/src/collection/server/dynamicIcon.ts`'s `computeCollectionIcon(collection, opts?)` (fails soft to `fallback ?? schema.icon`). |
