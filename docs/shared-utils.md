...
| `src/utils/html/customViewSrcdoc.ts`   | `buildCustomViewSrcdoc(html, {slug, token, dataUrl, origin})` | Build the sandboxed-iframe `srcdoc` for a collection custom view: injects the CSP meta + the `window.__MC_VIEW` bootstrap (scoped token + data URL) at the start of `<head>`.         |
