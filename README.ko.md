...
어시스턴트의 실체는 AI 모델이 아닙니다 — 모델은 엔진에 불과합니다. 어시스턴트의 가치를 결정하는 것은 당신에 대해 얼마나 알고 있는가입니다: 당신의 대화, 일정, 메모, 데이터, 그리고 사용하는 앱들. 그토록 중요한 것을 특정 서비스 제공자에게 맡겨서는 안 됩니다 — 오래 쓸수록 떠나기가 어려워지기 때문입니다. MulmoClaude는 오픈소스이며 로컬에서 실행되므로, 어시스턴트가 축적하는 모든 것 — 기억, 데이터, 앱 — 이 워크스페이스 안의 일반 파일로 당신 자신의 손에 남습니다.
...
그리고 어시스턴트는 책상 앞에서만 쓰는 것이 아닙니다. 스마트폰에서 — 또는 이미 쓰고 있는 메시징 앱에서 — 로그인하면, 컴퓨터 안에 살고 있는 같은 어시스턴트에 연결됩니다. 릴레이 서버는 메시지를 중계할 뿐, 데이터도 앱도 보관하지 않습니다.
