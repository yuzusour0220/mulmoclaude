      ...
      <section v-if="activeTab === 'review'" class="tab-pane">
        <div class="review-intro">
          ...
          <p class="text-muted text-sm">
            AI-on-a-Leash system captures all new profiles generated by the LLM as drafts. Approve them here to commit them to Markdown files.
          </p>
        </div>
...
async function loadClientDetails(clientId: string) {
  try {
    ...
    errorMsg.value = `Failed to load details for client: ${clientId}`;
...
async function updateClientStatus(newStatus: "active" | "paused" | "archived") {
  ...
  try {
    ...
    if (res?.ok) {
      successMsg.value = `Status updated to ${newStatus}`;
      await refreshAll();
    }
  } catch (err: any) {
    errorMsg.value = err.message || "Failed to update client status.";
  }
}
...
async function approveProjectCandidate(cand: ProjectCandidate) {
  ...
  try {
    ...
    if (res?.ok) {
      ...
      errorMsg.value = res?.error ?? "Failed to approve project candidate.";
    }
  } catch (err: any) {
    errorMsg.value = err.message || "Failed to approve project.";
  } finally {
    approving.value[cand.candidateId] = false;
  }
}
...
async function executeDeleteCandidate(candidateId: string, name: string) {
  ...
  try {
    ...
    if (res?.ok) {
      successMsg.value = `Deleted draft candidate "${name}".`;
      await refreshAll();
    } else {
...
async function submitProjectCandidate() {
  ...
  try {
    ...
    if (res?.ok) {
      successMsg.value = "Created project candidate draft! Approve below to commit.";
      showAddProjectForm.value = false;
      resetProjectForm();
      await refreshAll();
    } else {
      errorMsg.value = res?.error ?? "Failed to create project candidate.";
    }
  } catch (err: any) {
    errorMsg.value = err.message || "Failed to create project candidate.";
  } finally {
