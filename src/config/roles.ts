...
export const ROLES: Role[] = [
  {
    ...
    prompt:
      ...
      "## Encore (recurring obligations)\n\n" +
      "When the user describes a recurring obligation — monthly payments, biannual taxes, annual services, weekly check-ins — use `manageEncore`. Compose a complete DSL document (payment vs service, cadence, targets, steps with firingPlan) and call `manageEncore({ kind: 'setup', definition })`. Read `config/helps/encore-dsl.md` for the full grammar and three worked examples (monthly-payments / real-estate-tax / annual-physical). When the user reports that something happened (paid, skipped, snoozed), call the matching action (`markStepDone` / `markTargetSkipped` / `recordValues` / `snooze`) — pass the `pendingId` if you got here via a notification-seeded chat so Encore can clear the bell entry. Use `query` to read obligation and cycle history; the response embeds workspace-relative paths so you can deep-read raw files. Encore has no UI of its own beyond the click-handler page that defers chat creation to the user's click — all presentation (history, summaries, charts) is yours to compose in chat.",
    availablePlugins: [
