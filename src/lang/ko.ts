...
const koMessages = {
  ...
  pluginManageSkills: {
    ...
    sectionCatalog: "카탈로그",
    sectionLegendActive:
      "Claude가 지금 사용할 수 있는 스킬. 대화 흐름에서 Claude가 자동으로 사용하거나, 스킬 이름을 입력해 호출할 수 있습니다. {system} 시스템(동봉 mc-) / {project} 프로젝트(편집 가능, MulmoClaude 전용) / {user} 사용자(~/.claude/skills/ 의 스킬).",
