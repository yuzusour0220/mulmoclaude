...
- 選択中の result が無い (text 選択 or 何も選択なし) ときは `/chat/<sessionId>` のみ表示・コピー
...
1. `src/components/RightSidebar.vue` — 最上段に Permalink セクション追加
   ...
   - permalink computed: `selectedResultUuid` が ある時 `?result=<uuid>` を付与、無い時はセッション URL のみ、`sessionId` 自体無い時はセクションごと非表示
...
3. `src/lang/{en,ja,zh,ko,es,pt-BR,fr,de}.ts` — `rightSidebar` 配下に 3 keys 追加
   - `permalink` (セクションタイトル)
