...
| **L-24** `yarn sandbox:login` image 不在 | **要再定義** | plan の理解が古い。 現在の `yarn sandbox:login` は `security find-generic-password ... > ~/.claude/.credentials.json` の keychain export スクリプト (`package.json:63`) で、 docker image チェックロジックを持たない。 B-02 の元症状は別経路 (server boot → `ensureSandboxImage` in `server/system/docker.ts`) で発火するため、 シナリオ名と検証対象を redefine する必要。 現コードの 「image 不在時の auto-build」 を見るなら別途 spec 化、 もしくは unit test (`test/system/test_docker.ts` 新設) で十分 |
...
- **`MULMOCLAUDE_SANDBOX_IMAGE` env 新設** — `server/system/docker.ts:13` と `server/agent/config.ts:553` に `"mulmoclaude-sandbox"` がハードコード。 image 不在状況の test 再現には 「存在しない image 名 (`mulmoclaude-sandbox-e2e-nonexistent`) を env で指定」 経路が欲しい。 ただし B-02 (L-24) は現実装の `yarn sandbox:login` と乖離しているので、 シナリオ再定義が先 (「未実装シナリオの再評価」 参照)
...
- [ ] **`MULMOCLAUDE_SANDBOX_IMAGE` env 新設** — `server/system/docker.ts:13` と `server/agent/config.ts:553` でハードコード。 別 PR でソース改修すれば image 不在テストの再現が可能になる。 ただし B-02 (L-24) は plan 解釈との乖離で別途シナリオ再定義が必要
