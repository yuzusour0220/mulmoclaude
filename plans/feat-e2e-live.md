...
| L-17 | ui | notify tool 経由の publish はベルだけを更新し session-history unread badge は変えない (B-50) | ✅ |
...
#### L-17: notify tool 経由の publish はベルだけを更新し session-history unread badge は変えない (B-50) — ✅ 実装済

- カバー: B-50
- 重要度: **B** / Docker: `both` / 画像: 不要
- 元シナリオ: bridge から外部メッセージを送信 → 通知 bell バッジは更新されず、history バッジのみ更新
- 実装上の整形: bridge メッセージを e2e から直接走らせるには WebSocket 接続 + real LLM が必要で実用的でなかったため、 同じ B-50 不変条件を「`notify` MCP tool が発火する publish 経路から `session.hasUnread` に副作用が出ない」 という形で観測する canary に置き換えた
- 実装: (a) `server/agent/backend/fake-echo.ts` に `detectNotify` (`Use the notify tool with title "X"` 形 prompt の検出) と `dispatchNotifyInProcess` (notify.handler を in-process call) を追加 — production code path をそのまま使う形で fake-echo CI matrix でも動くように、 (b) `e2e-live/tests/ui.spec.ts` の L-17 spec: 初期 `[session-history-unread-badge]` snapshot → chat session 起こして `notify` tool 発火 prompt 送信 → assistant turn drain → `listNotifierEntries` で nonce title の entry を poll → bell panel 開いて `notification-item-<id>` visible → unread badge snapshot 不変 を assert → finally で clear + session delete
- production 追加面: **ゼロ**。 `00f4a740` の trust boundary (HTTP `publish` action 不在) を維持。 fake-echo の在不在は `MULMOCLAUDE_FAKE_AGENT` env で boot 時 1 回判定、 production 経路は触らない
- env 要件: **無し**。 `yarn dev` (real LLM) でも CI fake-echo でも env なしで走る
- 関連 testid: `[notification-bell]` / `[notification-item-<id>]` / `[session-history-unread-badge]` (本 PR で新設) / `[session-history-active-badge]` (本 PR で新設)
- 残課題: bridge end-to-end (実 WebSocket bridge 接続 + bridge origin agent run 完走) は scope 外。 やりたければ「test 専用 dev server spawn infra」 (Phase 2 #4) の上で別 L-XX として作る
...
| **L-17** notify tool 経由の publish 単独で session-history badge は変わらない (B-50) | ✅ 実装済 | ui.spec.ts、 `[session-history-unread-badge]` の `{visible,text}` snapshot を before 取得 → 新規 session で `Use the notify tool with title "e2e-live-l17-<nonce>"` を agent に送信 → assistant turn drain → `listNotifierEntries` を `expect.poll` で title 一致 entry が active に乗るまで待機 → bell panel 開いて `notification-item-<id>` が visible (pubsub → composable → render の通り全部踏んだ canary) → unread badge snapshot 不変を assert (B-50 regression: notifier publish と session.hasUnread が同じ event channel を共有していたら両方 tick する) → finally で clear + session delete。 fake-echo に `detectNotify` + `dispatchNotifyInProcess` (notify.handler を import して直接 call) を追加し、 fake-echo CI でも real LLM dev でも同じ prompt で動く。 production code path (notify → publishNotification → engine.publish → pubsub) をそのまま通すので production 追加面ゼロ、 env 不要。 bridge end-to-end を直接走らせる元シナリオは WebSocket + real LLM 必要で実用的でなかったため、 同じ不変条件を `notify` 経路で観測する整形。 `SessionHistoryToggleButton` の unread / active 両 badge に testid を新設し `docs/ui-cheatsheet.md` に節を追加 |
...
| **L-17** notifier 二重通知 (B-50) | ✅ **実装済** | fake-echo に `detectNotify` + `dispatchNotifyInProcess` (notify.handler を in-process 呼び出し) を追加し、 `ui.spec.ts` に「`notify` tool を発火させる prompt を agent に送る → bell row が visible → `[session-history-unread-badge]` 不変」 の canary を実装。 production code path (`notify` → `publishNotification` → `engine.publish` → pubsub → `useNotifications`) をそのまま通すので production 追加面ゼロ、 env 不要。 `00f4a740` の HTTP `publish` action 不在は維持。 bridge end-to-end の代わりに「`notify` 経由の publish 経路から session.hasUnread に副作用を出さない」 不変条件を観測する整形。 `SessionHistoryToggleButton` の unread / active 両 badge testid 新設と `docs/ui-cheatsheet.md` 節追加も同 PR |
...
3. ~~**L-17 二重通知 (B-50)**~~ — ✅ **実装済**。 (a) + (b) を 1 PR に統合: env-gated `publish` action 復活 + `ui.spec.ts` の inject canary。 「未実装シナリオ詳細 → L-17」 と 「未実装シナリオの再評価」 を参照
...
- ~~**L-17 用 notifier inject endpoint**~~ ✅ **解消済** — endpoint 追加せずに済んだ。 fake-echo の `detectNotify` + `dispatchNotifyInProcess` で notify.handler を直接 call する形で publish event 経路を踏ませた。 production HTTP route には何も追加していない (`00f4a740` の trust boundary 維持)、 env 不要
...
| **#1451** notifier-update-op | notifier 拡張、 update operation 追加 | L-17 (二重通知 canary) は `notify` MCP tool 経由で fyi publish しか観測しないので update op は scope 外。 update op 自体の canary が必要なら、 同じ「fake-echo に detector 追加 → in-process call」 パターンで update op を持つ既存 plugin (todo 等) 経由で観測する形が良い (production HTTP 追加面ゼロを維持できる) |
...
- [x] ~~**L-17 用 bridge メッセージ inject 経路の追加**~~: ✅ **解消済**。 当初検討した 3 案 (a) HTTP publish 復活、 (b) socket.io 直接 emit、 (c) 実 bridge WebSocket は全部不採用。 第 4 案として **fake-echo の `detectNotify` + `dispatchNotifyInProcess` 追加** で代替 — `notify` MCP tool の handler を fake-echo から直接 call する形で publish event 経路 (notify → publishNotification → engine.publish → pubsub) を full に通す canary を作った。 production HTTP route には何も追加しないので `00f4a740` の trust boundary が維持され、 env も不要。 実 bridge WebSocket end-to-end は将来 「test 専用 dev server spawn infra」 が立った後に別 L-XX として扱う
...
- [ ] **テスト専用 dev server 起動 infra** (案 C 拡張) — L-10 / L-13 / 階層 2 ベースのシナリオで `MULMOCLAUDE_WORKSPACE_PATH=/tmp/mc-e2e-<topic>` + `E2E_LIVE_BASE_URL` で隔離する仕組み。 上記 「Docker on/off 自動化検討」 案 C と同形 → 1 つの 「launcher 抽象化 + matrix」 PR にまとめる方向
